...
$res=callPost("http://".$urlB."/balanza/leerpeso.php",[],null);
echo json_encode($res);
break;
...
case 'getInvoicess_':
    ...
    if(isset($_SESSION['usert'])){
        ...
        query("insert into detalle_fpe (JSON, FechaRegistro, posi, user, url, JSON_datos) VALUES ('$res','$datoo', 1,'$usuario', '$url', '$data_datos')");
		$res=json_decode($res,TRUE);
       if(isset($res['error'])){
           echo json_encode(0);
       }else if(isset($res["DocNum"])){
           query("insert into eraser values(null,'$usuario','".$res["FederalTaxID"]."','".$res["U_NomFactura"]."',".$res["DocEntry"].",".$res["DocNum"].",null,null,null,null,null,'$fecha',1)");
           echo json_encode($res);
       }else{
           echo json_encode(0);
       }
...
break;
 ...
 //   $data=hanaquery("select * from FN_EXXIS_FE_CABECERA_BALANZA_BOLIVIA (''$DocEntry'',13,''--'')");
    echo json_encode($data);
    break;
    ...
    break;
        ...
        case 'FN_EXXIS_ESTADO1':
        $DocEntry=posted('DocEntry');
        $data=hanaquery("SELECT TX.\"DocEntry\", TX.\"U_EXX_FE_Estado\", TX.\"U_EXX_FE_Cuf\", TY.\"IdEstado\", TY.\"Nombre\",TX.\"U_EXX_FE_DESERR\"
        FROM OINV TX INNER JOIN \"FEX_BO\".\"FEX_ESTADOS\"  TY ON TX.\"U_EXX_FE_Estado\" = TY.\"Codigo\" WHERE TX.\"DocEntry\"  = ''$DocEntry''");
        echo json_encode($data);
        break;
